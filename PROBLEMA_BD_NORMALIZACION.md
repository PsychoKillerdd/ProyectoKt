...
- [x] Implementar función `normalizeHealthRecord()` en utils
- [x] Usar normalización en TODAS las escrituras a Firestore
- [x] Leer con fallback: `data.horasDeSueno || data.horasDeSueño`
- [x] Actualizar tipos/interfaces: solo definir `horasDeSueno`
- [ ] Agregar tests unitarios para normalización
- [x] Documentar estándar en código
- [x] Revisar código existente para inconsistencias
- [x] Probar con datos reales de Firestore
...
La aplicación Android ahora es **completamente compatible** con ambos formatos:

#### Archivos Modificados:

1. **DashboardComposeActivity.kt**
   - `loadHealthDataFromFirebase()`: Lee `horasDeSueno` con fallback a `horasDeSueño`
   - `loadSleepChartData()`: Compatible con ambos formatos para gráficos
   - `saveHealthDataToFirebase()`: Guarda usando estándar `horasDeSueno` (sin tilde)

2. **HistoryComposeActivity.kt**
   - `loadHistory()`: Lee ambos formatos para mostrar datos históricos correctamente

#### Compatibilidad Garantizada:

- ✅ **Lectura**: Soporta datos antiguos (con tilde) y nuevos (sin tilde)
- ✅ **Escritura**: Solo usa formato estándar `horasDeSueno` (sin tilde)
- ✅ **Gráficos**: Muestra datos correctamente sin importar el formato
- ✅ **Historial**: Lista todos los registros independientemente del formato
